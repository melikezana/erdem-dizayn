    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        <div className="mb-14 grid grid-cols-1 gap-8 border-b border-[#102B49]/10 pb-8 lg:grid-cols-12 lg:items-end">
          ...
          <p className="max-w-sm text-base leading-7 text-[#102B49]/70 lg:col-span-4">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-4">
          {PROCESS_STEPS.map((item) => (
            <article
              key={item.step}
              className="border-t border-[#102B49]/20 py-6 transition-colors hover:border-[#9A5C2F]"
            >
              <span className="mb-6 block font-serif text-4xl font-bold leading-none text-[#9A5C2F]/60">
                {item.step}
              </span>
              <h3 className="mb-3 font-serif text-2xl font-bold text-[#102B49]">
                {item.title}
              </h3>
              <p className="text-sm leading-7 text-[#102B49]/72">{item.desc}</p>
            </article>
          ))}
        </div>
      </div>
